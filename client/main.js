...
// ---------- Socket.IO ----------
// Use the full HTTPS URL so all clients connect to the same Azure backend
const socket = io("https://umes-vr-classroom-gteng2h7c4a7bwht.centralus-01.azurewebsites.net"); // backend server URL
...
function checkCollision(position, radius = 0.3) {
  ...
  for (const box of collisionBoxes) {
    const intersects =
      ...
      playerBox.min.y <= box.max.y &&
      playerBox.max.y >= box.min.y &&
...
function createClassroom() {
  ...
  scene.add(backWall);
  addCollisionBox(-10, 0, -7.6, 10, 4, -7.4);
  ...
  scene.add(frontWall);
  addCollisionBox(-10, 0, 7.4, 10, 4, 7.6);
  const leftWall = new THREE.Mesh(
    new THREE.BoxGeometry(0.2, 4, 15),
    wallMaterial,
  );
  leftWall.position.set(-10, 2, 0);
  scene.add(leftWall);
  addCollisionBox(-10.1, 0, -7.5, -9.9, 4, 7.5);
  ...
  scene.add(rightWall);
  addCollisionBox(9.9, 0, -7.5, 10.1, 4, 7.5);
  ...
  teacherDesk.position.set(0, 0.4, -6);
  scene.add(teacherDesk);
  addCollisionBox(-1, 0, -6.5, 1, 0.8, -5.5);
...
socket.on("playerMoved", ({ id, state }) => {
  if (id === socket.id) return;
  if (!remotes[id])
    remotes[id] = createRemotePlayer(scene, id, 0xff0000, state.position);
...
async function getLocalStream() {
  ...
  if (
    ...
    const msg =
      "getUserMedia not available. Serve over HTTPS or use a modern browser.";
...
function updateActiveCallsDisplay() {
  ...
  active.textContent = ids.length
    ? `Active: ${ids.map((i) => i.slice(0, 6)).join(", ")}`
    : "No active calls";
}
...
socket.on("webrtc-offer", async ({ from, offer }) => {
  ...
  const accept = confirm(
    `Incoming audio call from ${short}. Accept and share mic? Cancel to receive-only.`,
  );
...
function trySendState(now) {
  ...
  const state = {
    position: {
      ...
      y: local.mesh.position.y,
      z: local.mesh.position.z,
    },
    rotation: {
      x: local.mesh.quaternion.x,
      y: local.mesh.quaternion.y,
      z: local.mesh.quaternion.z,
      w: local.mesh.quaternion.w,
    },
  };
